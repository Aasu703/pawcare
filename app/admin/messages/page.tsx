          ...
          <div className="bg-white rounded-2xl p-6 w-full max-w-md">
            ...
            <div className="space-y-3 text-sm">
              ...
              <div className="border-t pt-3 mt-3">
                <p className="text-foreground">{selected.content}</p>
              </div>
            </div>
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-gray-100">
              {messages.length === 0 ? (
                ...
                <tr key={m._id} className="hover:bg-muted">
                  ...
                  <td className="px-6 py-4 text-sm text-muted-foreground">{m.createdAt ? new Date(m.createdAt).toLocaleDateString() : ""}</td>
                  <td className="px-6 py-4 text-right">
                    <div className="flex gap-1 justify-end">
                      <button onClick={() => setSelected(m)} className="p-2 text-muted-foreground hover:text-[var(--pc-teal)] hover:bg-[var(--pc-teal-light)] rounded-lg"><Eye className="h-4 w-4" /></button>
                      <button onClick={() => handleDelete(m._id)} className="p-2 text-muted-foreground hover:text-red-600 hover:bg-red-50 rounded-lg"><Trash2 className="h-4 w-4" /></button>
                    </div>
                  </td>
                </tr>
              ))}
