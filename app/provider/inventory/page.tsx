  ...
  if (!hasInventoryAccess) {
    return (
      <div className="bg-white rounded-xl border border-border p-8 text-center">
        <h1 className="text-2xl font-bold text-foreground mb-2">Inventory Not Available</h1>
    ...
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Track your supplies and equipment</p>
        </div>
        <button
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input type="number" value={form.price} onChange={(e) => setForm({ ...form, price: +e.target.value })}
                    className="w-full px-4 py-2.5 border border-border rounded-lg focus:ring-2 focus:ring-[var(--pc-teal)] focus:border-transparent" min="0" />
                </div>
              </div>
              <div>
      ...
      {loading ? (
        ...
        <div className="text-center py-20 bg-white rounded-xl border border-border">
          ...
          <p className="text-lg text-muted-foreground">No inventory items yet</p>
          <p className="text-sm text-muted-foreground mt-1">Start tracking your supplies</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {items.map((item) => {
            ...
            return (
              <div key={item._id} className="bg-white rounded-xl border border-border p-5">
                <div className="flex items-start justify-between">
                  <div>
                    <h3 className="font-semibold text-foreground">{item.product_name}</h3>
                    {item.description && <p className="text-xs text-muted-foreground mt-0.5">{item.description}</p>}
                    <p className="text-2xl font-bold text-[var(--pc-teal)] mt-2">
                      {item.quantity ?? 0} <span className="text-sm font-normal text-muted-foreground">units</span>
                    </p>
                  </div>
                  <div className="flex gap-1">
                    <button onClick={() => handleEdit(item)} className="p-2 text-muted-foreground hover:text-[var(--pc-teal)] hover:bg-[var(--pc-teal-light)] rounded-lg"><Pencil className="h-4 w-4" /></button>
                    <button onClick={() => handleDelete(item._id)} className="p-2 text-muted-foreground hover:text-red-600 hover:bg-red-50 rounded-lg"><Trash2 className="h-4 w-4" /></button>
                  </div>
                </div>
                {item.price ? <p className="text-xs text-muted-foreground mt-2">${item.price}</p> : null}
                {item.category && <p className="text-xs text-muted-foreground mt-1">Category: {item.category}</p>}
