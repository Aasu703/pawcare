...
export default function ProviderMessagesPage() {
  ...
  const selectedFromQuery = useMemo(() => {
    ...
    const participantRole = (searchParams.get("participantRole") || "user") as
      | "provider"
      | "user";
    const participantName = searchParams.get("participantName") || "Pet Owner";
  ...
  const participants = useMemo(() => {
    return contacts
      .filter((contact) => contact.participantRole === "user")
      .map((contact) => {
  ...
  return (
    <div>
      <div className="mb-8">
        ...
        <p className="text-muted-foreground mt-1">Chat with pet owners</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[320px,1fr] gap-4">
          <aside className="bg-white border border-border rounded-xl p-3">
            <h2 className="px-2 py-2 text-sm font-semibold text-foreground">Pet Owners</h2>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-xs text-muted-foreground truncate">{contact.subtitle || "User"}</p>
                      </div>
                    </button>
                  );
                })}
          ...
          <section className="bg-white border border-border rounded-xl flex flex-col min-h-[520px]">
            {!active ? (
              <div className="flex-1 flex flex-col items-center justify-center text-center p-8">
                ...
                <p className="text-lg text-muted-foreground">Select a contact to start chatting</p>
              </div>
            ) : (
              <>
                <div className="px-4 py-3 border-b border-border">
                  <p className="font-semibold text-foreground">{active.name}</p>
                  <p className="text-xs text-muted-foreground">{active.subtitle || "User"}</p>
                </div>
