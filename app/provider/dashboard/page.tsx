...
export default function ProviderDashboard() {
  ...
  const stats = isShop
    ? [
        { label: "Products", value: inventoryList.length, icon: Package, color: "bg-[var(--pc-teal)]" },
    ...
    : [
        { label: "Total Services", value: serviceList.length, icon: Wrench, color: "bg-[var(--pc-teal)]" },
  ...
  return (
    <div>
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-foreground">{getProviderTypeLabel(providerType)} Dashboard</h1>
        ...
        <AppNotificationBell
          audience="provider"
          providerType={providerType ?? undefined}
          buttonClassName="h-10 w-10 bg-white text-[var(--pc-teal)] border border-border hover:bg-muted"
      ...
      {loading ? (
        ...
        <>
          <div className={`grid grid-cols-1 ${isShop ? "md:grid-cols-3" : "md:grid-cols-2 xl:grid-cols-4"} gap-6 mb-10`}>
            {stats.map((s) => (
              <div key={s.label} className="bg-white rounded-xl border border-border p-6 flex items-center gap-4">
          ...
          <div className={`grid grid-cols-1 ${isShop ? "md:grid-cols-2" : "md:grid-cols-3"} gap-4`}>
            {canManageServices(providerType) && (
              <Link href="/provider/services" className="bg-white rounded-xl border border-border p-6 hover:shadow-md transition-shadow text-center">
                <Wrench className="h-8 w-8 text-[var(--pc-teal)] mx-auto mb-3" />
                <p className="font-semibold text-foreground">Manage Services</p>
                <p className="text-sm text-muted-foreground mt-1">Add or edit your services</p>
              </Link>
            )}
            {canManageBookings(providerType) && (
              <Link href="/provider/bookings" className="bg-white rounded-xl border border-border p-6 hover:shadow-md transition-shadow text-center">
                <CalendarCheck className="h-8 w-8 text-[var(--pc-teal)] mx-auto mb-3" />
                <p className="font-semibold text-foreground">Manage Bookings</p>
                <p className="text-sm text-muted-foreground mt-1">Confirm and process requests</p>
              </Link>
            )}
            {isVet && (
              <Link href="/provider/vet-appointments" className="bg-white rounded-xl border border-border p-6 hover:shadow-md transition-shadow text-center">
                <HeartPulse className="h-8 w-8 text-[var(--pc-teal)] mx-auto mb-3" />
                <p className="font-semibold text-foreground">Vet Appointments</p>
                <p className="text-sm text-muted-foreground mt-1">Create checkup reports for pets</p>
              </Link>
            )}
            {isVet && (
              <Link href="/provider/assigned-pets" className="bg-white rounded-xl border border-border p-6 hover:shadow-md transition-shadow text-center">
                <PawPrint className="h-8 w-8 text-[var(--pc-teal)] mx-auto mb-3" />
                <p className="font-semibold text-foreground">Assigned Pets</p>
                <p className="text-sm text-muted-foreground mt-1">Review pets assigned by users</p>
              </Link>
            )}
            {canManageBookings(providerType) && (
              <Link href="/provider/messages" className="bg-white rounded-xl border border-border p-6 hover:shadow-md transition-shadow text-center">
                <MessageSquare className="h-8 w-8 text-[var(--pc-teal)] mx-auto mb-3" />
                <p className="font-semibold text-foreground">Messages</p>
                <p className="text-sm text-muted-foreground mt-1">Chat with pet owners</p>
              </Link>
            )}
            {canManageInventory(providerType) && (
              <Link href="/provider/inventory" className="bg-white rounded-xl border border-border p-6 hover:shadow-md transition-shadow text-center">
                <Package className="h-8 w-8 text-[var(--pc-teal)] mx-auto mb-3" />
                <p className="font-semibold text-foreground">Manage Shop Products</p>
                <p className="text-sm text-muted-foreground mt-1">Add products visible to users</p>
              </Link>
            )}
            <Link href="/provider/feedback" className="bg-white rounded-xl border border-border p-6 hover:shadow-md transition-shadow text-center">
              <MessageSquare className="h-8 w-8 text-[var(--pc-teal)] mx-auto mb-3" />
              <p className="font-semibold text-foreground">View Feedback</p>
              <p className="text-sm text-muted-foreground mt-1">See customer feedback</p>
            </Link>
          </div>
          ...
          {!isShop && serviceList.length > 0 && (
            <div className="mt-10">
              <h2 className="text-xl font-semibold text-foreground mb-4">Your Services</h2>
                ...
                <table className="w-full">
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {serviceList.slice(0, 5).map((s, idx) => (
                      <tr key={s._id ?? `service-${idx}`} className="hover:bg-muted">
                        <td className="px-6 py-4 text-sm font-medium text-foreground">{s.title}</td>
                        <td className="px-6 py-4 text-sm text-muted-foreground capitalize">{s.category || s.catergory || "-"}</td>
                        <td className="px-6 py-4 text-sm text-muted-foreground">${s.price}</td>
                ...
                <table className="w-full">
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {inventoryList.slice(0, 5).map((item, idx) => (
                      <tr key={item._id ?? `inventory-${idx}`} className="hover:bg-muted">
                        ...
                        <td className="px-6 py-4 text-sm text-muted-foreground">${item.price || 0}</td>
                        <td className="px-6 py-4 text-sm text-muted-foreground">{item.quantity || 0}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
                ...
                <table className="w-full">
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {bookingList.slice(0, 5).map((booking, idx) => (
                      <tr key={booking._id ?? `booking-${idx}`} className="hover:bg-muted">
                        <td className="px-6 py-4 text-sm text-foreground">#{(booking._id || "").slice(-6)}</td>
                        <td className="px-6 py-4 text-sm text-muted-foreground">{booking.service?.title || "Service"}</td>
                        <td className="px-6 py-4 text-sm text-muted-foreground">{new Date(booking.startTime).toLocaleDateString()}</td>
                        <td className="px-6 py-4 text-sm text-muted-foreground capitalize">{booking.status}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
